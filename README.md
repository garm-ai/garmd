...
**The governed runtime.** A policy-enforcing proxy between an agent and the
tools it may call, deciding on every call what passes and what the caller is
allowed to see of the answer.

A long-running process. Systemd or Kubernetes starts it; nobody types it. The
command line tool is [`garm`](../garm).
...
| | |
|---|---|
| `cmd/garmd/` | The daemon: the generation API and the tool plane, on separate listeners |
...
- **The annotations and the generators** — [`garm`](../garm).
- **Anything a tool service imports.** A tool must not be able to reach this
  ...
  enforcement is the failure this boundary prevents. See
  [`tool-go`](../tool-go) and [`tool-python`](../tool-python).
- **Any tool.** This repository must never build-depend on one. A catalogue
  crosses that line as data, never as an import.

## Status

Not yet seeded. Intent recorded; code arrives after the CLI is extracted.
